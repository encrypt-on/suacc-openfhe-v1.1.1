                             ...
                             )
{
    ...
    if(!cached_init){
        if(xdir == XFORMDIR::BOTH) {
            forward_root_of_unity_table_generator();
            inverse_root_of_unity_table_generator();
        }else if(xdir == XFORMDIR::FWD){
            forward_root_of_unity_table_generator();
        }else if(xdir == XFORMDIR::REV){
            inverse_root_of_unity_table_generator();
        }
    }
...
void NTTParameters::forward_root_of_unity_table_generator() //704::TODO optimize it
{
    forward_root_of_unity_table.push_back(1);

    for (int i = 1; i < root_of_unity_size; i++)
    {
        Data exp = VALUE::mult(forward_root_of_unity_table[i - 1], root_of_unity, modulus);
