...
void somethrustdemo () {
    ...
    thrust::host_vector<int> h_input(size, 3); // Initialize with integer value 3
    thrust::host_vector<int> h_result(size);

    // Initialize device vectors
    thrust::device_vector<int> d_input = h_input;
    thrust::device_vector<int> d_result(size);

    // Perform element-wise doubling using LibThrust
    thrust::transform(d_input.begin(), d_input.end(), d_result.begin(), double_functor());
...
std::vector<Root_> gpuntt_rou_table_generator(const NTTParameters& nttp, std::vector<Data> table)
{
    // Taking Bitreverse order of root of unity table
#if defined(BARRETT_64) || defined(GOLDILOCKS_64)
    
    std::vector<Data> new_table;
    int lg = log2(nttp.root_of_unity_size);
    for (int i = 0; i < nttp.root_of_unity_size; i++)
    {
    ...
    #error __FUNCTION__ ## "Unimplemented, TODO for 704"
#else
